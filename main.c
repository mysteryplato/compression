 ...
 *  @brief 模拟一个图像压缩算法：
 *         1.生成数据：数据为随机单精度浮点型
 *         2.压缩数据：将数据转换为字符型并写入到文件中
 *         3.解压缩数据：读取压缩的文件，将字符型数据转换回单精度浮点型数据
 *         4.验证数据：计算出原始数据与解压缩后的数据的差值
 *  @author plato
 *  @version 1.00 2017-11-07 plato
 *                note:create it
 */
...
int main()
{
    float *pFloatData;                               // 存放生成的数据的头指针
    short arrLenArr[ARR_COUNT];                      // 存放随机生成的数组长度

    pFloatData = getpData(arrLenArr);                // 生成随机数据

    compressData(pFloatData,arrLenArr);              // 压缩数据
    decompressData(pFloatData,arrLenArr);            // 解压缩数据

    free(pFloatData);                                // 释放开辟的内存空间
    pFloatData = NULL;                               // 将指针初始化为NULL
    return 0;
}
